<template>
  <section class="compras">
    <header class="compras__header">
      <div class="compras__headings">
        ...
        <p class="compras__subtitle">
          Controla las adquisiciones de productos. Tienes
          <strong>{{ totalCompras }}</strong>
          compras registradas con un total acumulado de
          <strong>{{ formatCurrency(totalInvertido) }}</strong>.
        </p>
      </div>
    ...
    <div v-if="isLoading" class="compras__state">
      <span class="compras__state-text">Cargando compras...</span>
    </div>
    ...
    <div v-else-if="comprasOrdenadas.length === 0" class="compras__state">
      <span class="compras__state-text">
        Aún no registras compras. Usa el botón superior para añadir la primera.
      </span>
    </div>
    ...
    <div v-else class="compras__table-wrapper">
      <table class="compras__table">
        ...
        <tbody>
          <tr v-for="compra in comprasOrdenadas" :key="compra.id">
            <td>
              <div>{{ formatDate(compra.fecha) }}</div>
              <small class="compras__small-text">{{ renderHora(compra.fecha) }}</small>
            </td>
            <td>
            ...
            <td>
              <div class="compras__productos">
                {{ describirProductos(compra.items) }}
              </div>
            </td>
            <td class="compras__column--numeric">
              {{ contarArticulos(compra.items) }}
            </td>
            <td class="compras__column--numeric">
...
export default {
  ...
  setup() {
    ...
    const irACrear = () => {
      router.push({ name: 'ComprasCrear' })
    }

    const reintentar = () => {
      cargarCompras({ force: true })
    }
